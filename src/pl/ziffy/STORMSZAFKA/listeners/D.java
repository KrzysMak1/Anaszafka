...
public class D implements Listener
{
    ...
    public D() {
        this.B = new HashSet<UUID>();
        this.A = (JavaPlugin)Bukkit.getPluginManager().getPlugin("STORMSZAFKA");
        Bukkit.getScheduler().runTaskTimer((Plugin)this.A, () -> {
            ...
            while (iterator.hasNext()) {
                ...
                if (player.getOpenInventory().getType() == InventoryType.CRAFTING && !player.hasMetadata("inventory_open_main") && !player.hasMetadata("right_clicked")) {
                    player.setMetadata("inventory_open_main", (MetadataValue)new FixedMetadataValue((Plugin)this.A, (Object)true));
                }
            }
        }, 5L, 5L);
    }
    ...
    public void onPlayerInteract(final PlayerInteractEvent playerInteractEvent) {
        ...
        if (playerInteractEvent.getAction() == Action.RIGHT_CLICK_AIR || playerInteractEvent.getAction() == Action.RIGHT_CLICK_BLOCK) {
            player.setMetadata("right_clicked", (MetadataValue)new FixedMetadataValue((Plugin)this.A, (Object)true));
            Bukkit.getScheduler().runTaskLater((Plugin)this.A, () -> {
                if (player2.isOnline() && player2.hasMetadata("right_clicked")) {
                    player2.removeMetadata("right_clicked", (Plugin)this.A);
                }
            }, 5L);
        }
    }
    ...
    public void onInventoryClose(final InventoryCloseEvent inventoryCloseEvent) {
        ...
        if (inventoryCloseEvent.getView().getType() == InventoryType.CRAFTING && player.hasMetadata("inventory_open_main")) {
            player.removeMetadata("inventory_open_main", (Plugin)this.A);
        }
    }
}
