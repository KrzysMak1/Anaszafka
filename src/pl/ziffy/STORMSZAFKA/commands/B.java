...
import pl.ziffy.STORMSZAFKA.costumes.G;
...
public class B implements CommandExecutor, TabCompleter
{
    ...
    public boolean onCommand(final CommandSender commandSender, final Command command, final String s, final String[] array) {
        if (command.getName().equalsIgnoreCase("STORMSZAFKA")) {
            if (array.length >= 1) {
                if (this.A.A(commandSender, array)) {
                    return true;
                }
                if (array[0].equalsIgnoreCase("resetcostume") && array.length >= 2) {
                    ...
                    if (player == null) {
                        commandSender.sendMessage(this.A("&8» &7Gracz &f" + array[1] + " &7nie jest &#FF0000online&7!"));
                        return true;
                    }
                    if (L.F(player)) {
                        L.C(player);
                    ...
                    this.B.D(player);
                    commandSender.sendMessage(this.A("&8» &7Zresetowano dane kostiumu dla gracza &f" + player.getName() + " &#27FF00sukces&7!"));
                    player.sendMessage(this.A("&8» &7Zresetowano twoje dane &#27FF00kostiumu&7!"));
                    return true;
                }
                    ...
                    commandSender.sendMessage(this.A("&8» &7Sprawdzanie i naprawianie uszkodzonych danych &#FFD700kostium\u00f3w&7..."));
                    final int e = this.B.E();
                    if (e > 0) {
                        commandSender.sendMessage(this.A("&8» &7Naprawiono &f" + e + " &7uszkodzonych wpis\u00f3w danych &#27FF00kostium\u00f3w&7!"));
                    }
                    else {
                        commandSender.sendMessage(this.A("&8» &7Nie znaleziono uszkodzonych danych &#27FF00kostium\u00f3w&7."));
                    }
                    return true;
                }
            }
        ...
        if (command.getName().equalsIgnoreCase("STORMSZAFKA")) {
            ...
            if (array.length >= 2) {
                if (array[0].equalsIgnoreCase("give")) {
                    final List<String> b = this.A.B(commandSender, array);
                    if (!b.isEmpty()) {
                        return b;
                    }
                }
