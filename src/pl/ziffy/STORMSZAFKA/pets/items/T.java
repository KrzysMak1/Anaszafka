...
public class T implements D, Listener
{
    ...
    private void V() {
        final Map<String, Object> d = this.\u0200.D("duszek");
        this.\u0207 = (double)d.get("height");
        this.\u0209 = (double)d.get("speed");
        this.\u0203 = (double)d.get("distance");
        final Map<String, Object> d2 = this.\u0200.D("duszek", "speed");
        this.\u0201 = (int)d2.get("level");
        this.\u0202 = (boolean)d2.get("visible");
        final Map<String, Object> d3 = this.\u0200.D("duszek", "dripstone_protection");
        this.\u0204 = (double)(d3.containsKey("reduction") ? d3.get("reduction") : 0.5);
        final Map<String, Object> d4 = this.\u0200.D("duszek", "health");
    ...
    public void spawn(final Player \u01fb, final String s) {
        ...
        String string;
        if (this.\u0200 != null) {
            final Object b = this.\u0200.B("duszek", "item.texture.value");
            if (b instanceof final String s2) {
                string = s2;
            }
            else {
                string = this.\u0200.G("duszek").getString("texture.value", "e3RleHR1cmVzOntTS0lOOnt1cmw6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvZWFhMWE2MTk5NjBhYzY4NjBjYjVjOWI2ZDNjODg3NDVkNmVmNjQwMzAwOWFmNWJhMGQwODM4Yjg3YjEyZGYwNCJ9fX0=");
            }
        }
        else {
        ...
        E.B(this.\u01fd = (ArmorStand)\u01fb.getWorld().spawn(add, (Class)ArmorStand.class, armorStand -> {
            ...
            itemMeta.setDisplayName(G.C(this.\u0200.L("duszek") + " &8\u25cf &f" + \u01fb.getName()));
            this.S(itemMeta, string);
            helmet.setItemMeta((ItemMeta)itemMeta);
            armorStand.setHelmet(helmet);
        }), \u01fb.getUniqueId());
        final Map<String, Object> a = this.\u0200.A("duszek");
        E.A(this.\u020b = (ArmorStand)\u01fb.getWorld().spawn(add.clone().add(0.0, (double)a.get("height"), 0.0), (Class)ArmorStand.class, armorStand -> {
            armorStand.setVisible(false);
            armorStand.setGravity(false);
            armorStand.setMarker(true);
            armorStand.setCustomName(G.C(s + " &8\u25cf &f" + \u01fb.getName()));
            armorStand.setCustomNameVisible(true);
        }), \u01fb.getUniqueId());
        \u01fb.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, Integer.MAX_VALUE, this.\u0201 - 1, false, this.\u0202));
        try {
            \u01fb.getAttribute(Attribute.GENERIC_MAX_HEALTH).getModifiers().stream().filter(attributeModifier -> attributeModifier.getUniqueId().equals(T.\u0206)).forEach(attributeModifier2 -> player.getAttribute(Attribute.GENERIC_MAX_HEALTH).removeModifier(attributeModifier2));
            \u01fb.getAttribute(Attribute.GENERIC_MAX_HEALTH).addModifier(new AttributeModifier(T.\u0206, "duszek_health_bonus", this.\u01fb, AttributeModifier.Operation.ADD_NUMBER));
    ...
    public void update() {
        ...
        this.\u01fd.teleport(add);
        this.\u020b.teleport(add.clone().add(0.0, (double)this.\u0200.A("duszek").get("height"), 0.0));
        if (hasPotionEffect) {
    ...
    public void remove() {
        ...
        try {
            if (this.\u01fa != null && this.\u01fa.isOnline()) {
                this.\u01fa.getAttribute(Attribute.GENERIC_MAX_HEALTH).getModifiers().stream().filter(attributeModifier -> attributeModifier.getUniqueId().equals(T.\u0206)).forEach(attributeModifier2 -> this.\u01fa.getAttribute(Attribute.GENERIC_MAX_HEALTH).removeModifier(attributeModifier2));
    ...
    public String getDisplayName() {
        return this.\u0200.G("duszek").getString("name", "Duszek");
    }
    ...
    private void S(final SkullMeta skullMeta, final String s) {
        C.D(skullMeta, s);
    }
