...
public class D implements Listener
{
    ...
    public static void openLoreEditor(final Player player, final String s) {
        final Inventory inventory = Bukkit.createInventory((InventoryHolder)null, 45, "§x§F§F§F§9§1§A\u26a1 §8\u1d07\u1d05\u028f\u1d04\u1d0a\u1d00 \u1d18\u1d07\u1d1b\u1d00 " + B(s) + " \u029f\u1d0f\u0280\u1d07");
        D.C.put(player.getUniqueId(), s);
        final List<String> c = C(s);
        final int[] array = { 10, 11, 12, 13, 14, 15, 16, 19, 20, 21, 22, 23, 24, 25 };
        for (int n = 0; n < c.size() && n < array.length; ++n) {
            inventory.setItem(array[n], A((String)c.get(n), n + 1));
        }
        ...
        if (b) {
            itemMeta2.setDisplayName(A("&#1DFF1ADodaj linijk\u0119 lore"));
        }
        else {
            itemMeta2.setDisplayName(A("&7Dodaj linijk\u0119 lore"));
            final ArrayList lore = new ArrayList();
            lore.add(A("&cOsi\u0105gni\u0119to maksymaln\u0105 liczb\u0119 linijek (14)"));
    ...
    private static ItemStack A(final String s, final int n) {
        ...
        final ItemMeta itemMeta = itemStack.getItemMeta();
        itemMeta.setDisplayName(A("&#FFD700Linijka " + n));
        final ArrayList lore = new ArrayList();
        lore.add(A("&7"));
        lore.add(A("&7Tre\u015b\u0107: &f" + s));
        lore.add(A("&7"));
        lore.add(A("&7\u1d0b\u029f\u026a\u1d0b\u0274\u026a\u1d0a &#08B8FB&n\u029f\u1d07\u1d21\u028f\u1d0d&7, \u1d00\u0299\u028f \u1d18\u0280\u1d22\u1d07\ua731\u1d1c\u0274\u1d00\u0107 \u1d21 \u0262\u00f3\u0280\u1d07!"));
        lore.add(A("&7\u1d0b\u029f\u026a\u1d0b\u0274\u026a\u1d0a &#21FB08&n\u1d18\u0280\u1d00\u1d21\u028f\u1d0d&7, \u1d00\u0299\u028f \u1d18\u0280\u1d22\u1d07\ua731\u1d1c\u0274\u1d00\u0107 \u1d21 \u1d05\u00f3\u029f!"));
        lore.add(A("&7\u1d0b\u029f\u026a\u1d0b\u0274\u026a\u1d0a &#FF0000&n\ua731\u029c\u026a\ua730\u1d1b + \u1d18\u0280\u1d00\u1d21\u028f&7, \u1d00\u0299\u028f \u1d22\u0280\u1d07\ua731\u1d07\u1d1b\u1d0f\u1d21\u1d00\u0107!"));
    ...
    private static List<String> C(final String s) {
        final FileConfiguration d = pl.ziffy.STORMSZAFKA.pets.editors.A.D(s);
        if (d == null) {
    ...
    private static void A(final String s, final List<String> list) {
        final FileConfiguration d = pl.ziffy.STORMSZAFKA.pets.editors.A.D(s);
        if (d == null) {
            return;
        }
        d.set("item.lore", (Object)list);
        pl.ziffy.STORMSZAFKA.pets.editors.A.A(s, d);
    }
    ...
    public void onInventoryClick(final InventoryClickEvent inventoryClickEvent) {
        ...
        final String s = D.C.get(player.getUniqueId());
        if (s == null) {
            return;
        }
        final int rawSlot = inventoryClickEvent.getRawSlot();
        if (rawSlot == 39) {
            D.C.remove(player.getUniqueId());
            pl.ziffy.STORMSZAFKA.pets.editors.B.openPetEditor(player, s);
            return;
        }
        if (rawSlot == 41 && currentItem.getType() == Material.LIME_DYE) {
            D.A.put(player.getUniqueId(), "add");
            player.closeInventory();
            player.sendMessage(A("&8» &7Wpisz &#FFD300now\u0105 &7linijk\u0119 lore &fpeta &8(&7Wpisz &#FF0000anuluj&7, aby anulowa\u0107&8)&7!"));
            player.sendTitle(A(""), A("&7Wpisz &#FFD300now\u0105 &7linijk\u0119 lore na &fchacie&7!"), 10, 60, 15);
        ...
        if (n != -1 && currentItem.getType() == Material.PAPER) {
            ...
            if (inventoryClickEvent.getClick() == ClickType.SHIFT_RIGHT) {
                ...
                player.sendMessage(A("&8» &7Linijka lore zosta\u0142a &#FF0000usuni\u0119ta&7!"));
                player.sendTitle(A(""), A("&fLinijka &7zosta\u0142a &#FF0000usuni\u0119ta&7!"), 10, 60, 15);
    ...
    public void onPlayerChat(final AsyncPlayerChatEvent asyncPlayerChatEvent) {
        ...
        if (!D.A.containsKey(uniqueId)) {
            return;
        }
        asyncPlayerChatEvent.setCancelled(true);
        final String s = D.A.remove(uniqueId);
        final String s2 = D.C.get(uniqueId);
        ...
        if (message.equalsIgnoreCase("anuluj")) {
            Bukkit.getScheduler().runTask((Plugin)Main.getInstance(), () -> {
                ...
                player.sendTitle(A(""), A("&fLinijka lore &7zosta\u0142a &#FF0000anulowana&7!"), 10, 60, 15);
                E(player, "ENTITY_ENDERMAN_TELEPORT");
            });
            return;
        }
        if (s.equals("add")) {
            final List<String> c = C(s2);
            if (c.size() >= 14) {
                Bukkit.getScheduler().runTask((Plugin)Main.getInstance(), () -> {
                    openLoreEditor(player2, s4);
                    player2.sendMessage(A("&8» &7Osi\u0105gni\u0119to maksymaln\u0105 liczb\u0119 &#FF0000linijek&7!"));
                    player2.sendTitle(A(""), A("&fLinijka lore &7osi\u0105gn\u0119\u0142a &#FF0000limit&7!"), 10, 60, 15);
            ...
            Bukkit.getScheduler().runTask((Plugin)Main.getInstance(), () -> {
                ...
                player3.sendMessage(A("&8» &7Dodano now\u0105 &flinijk\u0119 &7lore &#27FF00pomy\u015blnie&7!"));
                player3.sendTitle(A(""), A("&fLinijka lore &7zosta\u0142a &#27FF00dodana&7!"), 10, 60, 15);
                E(player3, "ENTITY_PLAYER_LEVELUP");
            });
        }
        else if (s.startsWith("edit_")) {
            final int int1 = Integer.parseInt(s.split("_")[1]);
            final List<String> c2 = C(s2);
            if (int1 >= c2.size()) {
                return;
            }
            c2.set(int1, message);
            ...
            Bukkit.getScheduler().runTask((Plugin)Main.getInstance(), () -> {
                ...
                player4.sendMessage(A("&8» &7Linijka lore zosta\u0142a &#27FF00zedytowana&7!"));
                player4.sendTitle(A(""), A("&fLinijka lore &7zosta\u0142a &#27FF00zedytowana&7!"), 10, 60, 15);
