...
public class A implements Listener
{
    ...
    private static final String E = "&x&F&F&F&9&1&A\u26a1 &8\u1d07\u1d05\u028f\u1d04\u1d0a\u1d00 \u1d18\u0280\u1d22\u1d07\u1d05\u1d0d\u026a\u1d0f\u1d1b\u00f3\u1d21";
    ...
    public static void openEditorMenu(final Player player, final int i, final int j) {
        pl.ziffy.STORMSZAFKA.ui.gui.A.A.put(player.getUniqueId(), i);
        pl.ziffy.STORMSZAFKA.ui.gui.A.C.put(player.getUniqueId(), j);
        final Inventory inventory = Bukkit.createInventory((InventoryHolder)null, 54, ChatColor.translateAlternateColorCodes('&', C("&x&F&F&F&9&1&A\u26a1 &8\u1d07\u1d05\u028f\u1d04\u1d0a\u1d00 \u1d18\u0280\u1d22\u1d07\u1d05\u1d0d\u026a\u1d0f\u1d1b\u00f3\u1d21")));
        final int[] array = { 10, 11, 12, 19, 20, 21 };
        final int length = array.length;
        final int n = (i - 1) * length;
        for (int min = Math.min(n + length, pl.ziffy.STORMSZAFKA.ui.gui.A.B.size()), k = n; k < min; ++k) {
            inventory.setItem(array[k - n], B(pl.ziffy.STORMSZAFKA.ui.gui.A.B.get(k)));
        }
        final int[] array2 = { 14, 15, 16, 23, 24, 25 };
        final int length2 = array2.length;
        final int n2 = (j - 1) * length2;
        for (int min2 = Math.min(n2 + length2, pl.ziffy.STORMSZAFKA.ui.gui.A.D.size()), l = n2; l < min2; ++l) {
            inventory.setItem(array2[l - n2], E(pl.ziffy.STORMSZAFKA.ui.gui.A.D.get(l)));
        }
    ...
    private static ItemStack B(final String s) {
        final ItemStack a = pl.ziffy.STORMSZAFKA.pets.E.A(s, "brak", false);
    ...
    private static ItemStack E(final String s) {
        final ItemStack a = L.A(s, "brak", false);
        if (a != null && a.hasItemMeta()) {
            final ItemMeta itemMeta = a.getItemMeta();
            final ArrayList lore = itemMeta.hasLore() ? new ArrayList(itemMeta.getLore()) : new ArrayList();
            lore.removeIf(s2 -> ChatColor.stripColor(s2).contains("Kliknij prawym"));
    ...
    public void onInventoryClick(final InventoryClickEvent inventoryClickEvent) {
        ...
        if (!inventoryClickEvent.getView().getTitle().equals(ChatColor.translateAlternateColorCodes('&', C("&x&F&F&F&9&1&A\u26a1 &8\u1d07\u1d05\u028f\u1d04\u1d0a\u1d00 \u1d18\u0280\u1d22\u1d07\u1d05\u1d0d\u026a\u1d0f\u1d1b\u00f3\u1d21")))) {
            return;
        }
        ...
        final int intValue = pl.ziffy.STORMSZAFKA.ui.gui.A.A.getOrDefault(player.getUniqueId(), 1);
        final int intValue2 = pl.ziffy.STORMSZAFKA.ui.gui.A.C.getOrDefault(player.getUniqueId(), 1);
        final int n = (int)Math.ceil(pl.ziffy.STORMSZAFKA.ui.gui.A.B.size() / 6.0);
        final int n2 = (int)Math.ceil(pl.ziffy.STORMSZAFKA.ui.gui.A.D.size() / 6.0);
        if (rawSlot == 28) {
            int n3 = intValue - 1;
            if (n3 < 1) {
                n3 = n;
        ...
        if (b) {
            ...
            if (n8 < pl.ziffy.STORMSZAFKA.ui.gui.A.B.size()) {
                final String s = pl.ziffy.STORMSZAFKA.ui.gui.A.B.get(n8);
                ...
                if (inventoryClickEvent.getClick() == ClickType.SHIFT_RIGHT) {
                    pl.ziffy.STORMSZAFKA.pets.editors.A.F(s);
        ...
        if (b2) {
            ...
            if (n10 < pl.ziffy.STORMSZAFKA.ui.gui.A.D.size()) {
                final String s2 = pl.ziffy.STORMSZAFKA.ui.gui.A.D.get(n10);
                if (inventoryClickEvent.getClick() == ClickType.RIGHT) {
                    final ItemStack a2 = L.A(s2, "brak", false);
                    if (a2 != null) {
                        if (player.getInventory().firstEmpty() != -1) {
                ...
                if (inventoryClickEvent.getClick() == ClickType.SHIFT_RIGHT) {
                    pl.ziffy.STORMSZAFKA.costumes.editors.D.D(s2);
                ...
                if (inventoryClickEvent.getClick() == ClickType.LEFT) {
                    pl.ziffy.STORMSZAFKA.costumes.editors.A.openCostumeEditor(player, s2);
                }
            }
        }
    }
