...
public abstract class CostumesCostumeService5 implements CostumesCostumeService1
{
    protected static final Map<UUID, CostumesCostumeService4> playerArmorMap;
    protected static final Map<UUID, Long> doubleJumpCooldowns;
    protected final Plugin plugin;
    protected final String costumeType;
    protected final String displayName;
    
    public CostumesCostumeService5(final String costumeType, final String displayName) {
    ...
    protected void takeArmorSnapshot(final Player player) {
        CostumesCostumeService5.playerArmorMap.put(player.getUniqueId(), new CostumesCostumeService4(player.getInventory().getHelmet(), player.getInventory().getChestplate(), player.getInventory().getLeggings(), player.getInventory().getBoots()));
    }
    
    protected void restoreArmorFromSnapshot(final Player player) {
        final CostumesCostumeService4 i = CostumesCostumeService5.playerArmorMap.get(player.getUniqueId());
        if (i != null) {
            player.getInventory().setHelmet(i.CostumesCostumeService1());
            player.getInventory().setChestplate(i.CostumesCostumeService2());
            player.getInventory().setLeggings(i.CostumesListener2());
            player.getInventory().setBoots(i.CostumesListener1());
        }
    }
    ...
    protected String formatColors(String a) {
        ...
        a = this.CostumesListener1(a);
        return ChatColor.translateAlternateColorCodes('&', a);
    }
    
    private String CostumesListener1(final String s) {
        if (s == null) {
            return null;
        }
        final Matcher matcher = Pattern.compile("&#([CostumesListener1-Fa-f0-9]{6})").matcher(s);
    ...
    public static boolean hasCostume(final Player player) {
        return CostumesCostumeService5.playerArmorMap.containsKey(player.getUniqueId());
    }
    
    public static CostumesCostumeService4 getPlayerArmorSnapshot(final UUID uuid) {
        return CostumesCostumeService5.playerArmorMap.get(uuid);
    }
    
    public static void storeArmorSnapshot(final UUID uuid, final CostumesCostumeService4 i) {
        CostumesCostumeService5.playerArmorMap.put(uuid, i);
    }
    ...
    static {
        playerArmorMap = new HashMap<UUID, CostumesCostumeService4>();
