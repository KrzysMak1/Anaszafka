...
public class D implements Listener
{
    ...
    public void onInventoryClick(final InventoryClickEvent inventoryClickEvent) {
        ...
        if (b && cursor != null && cursor.hasItemMeta() && this.C(cursor)) {
            inventoryClickEvent.setCancelled(true);
            return;
        }
        if ((inventoryClickEvent.getClick() == ClickType.SHIFT_LEFT || inventoryClickEvent.getClick() == ClickType.SHIFT_RIGHT) && currentItem != null && currentItem.hasItemMeta() && this.C(currentItem)) {
            inventoryClickEvent.setCancelled(true);
            return;
        }
        if (inventoryClickEvent.getClick() == ClickType.NUMBER_KEY && b) {
            ...
            if (item != null && item.hasItemMeta() && this.C(item)) {
                inventoryClickEvent.setCancelled(true);
            }
        }
    }
    ...
    public void onInventoryClickMonitor(final InventoryClickEvent inventoryClickEvent) {
        ...
        this.B((Player)inventoryClickEvent.getWhoClicked());
    }
    ...
    public void onPlayerInteract(final PlayerInteractEvent playerInteractEvent) {
        ...
        if (item.getType().name().contains("SHULKER_BOX") && "wampir".equals(L.D(player))) {
            return;
        }
        if (!this.C(item)) {
            return;
        ...
        playerInteractEvent.setCancelled(true);
        this.B(player);
        this.A.getServer().getScheduler().scheduleSyncDelayedTask((Plugin)this.A, () -> this.B(player2), 1L);
        ItemStack e = null;
        if (L.F(player)) {
            if (!this.A.getConfig().getBoolean("activation.allow_costume_swap", false)) {
                pl.ziffy.STORMSZAFKA.messages.A.E().A(player, "costume.already-active-item");
                return;
            }
            e = L.E(player);
        ...
        if (a != null && this.A(a)) {
            pl.ziffy.STORMSZAFKA.messages.A.E().A(player, "costume.expired-item");
        ...
        final String d = this.D(item);
        if (d == null) {
            pl.ziffy.STORMSZAFKA.messages.A.E().A(player, "costume.invalid-type-item");
            if (e != null) {
                L.A(player, e);
            }
            return;
        }
        final String a2 = this.A(item);
        final String b = this.B(item);
        boolean b2;
        if (a2 != null) {
            b2 = L.A(player, d, b, a2);
        }
        else {
            b2 = L.A(player, d, b);
        }
        if (b2) {
            ...
            pl.ziffy.STORMSZAFKA.messages.A.E().A(player, "costume.activation-failed");
            if (e != null) {
                L.A(player, e);
            }
        }
    ...
    private boolean C(final ItemStack itemStack) {
        ...
        try {
            if (itemMeta.getPersistentDataContainer().has(new NamespacedKey((Plugin)this.A, "costume_type"), PersistentDataType.STRING)) {
                return true;
            }
        }
    ...
    private String D(final ItemStack itemStack) {
        if (!itemStack.hasItemMeta()) {
            return null;
        }
        final ItemMeta itemMeta = itemStack.getItemMeta();
        try {
            ...
            final NamespacedKey namespacedKey = new NamespacedKey((Plugin)this.A, "costume_type");
    ...
    private boolean A(final String text) {
        try {
    ...
    private void B(final Player player) {
        ...
        if (helmet != null && this.C(helmet)) {
            player.getInventory().setHelmet((ItemStack)null);
        }
        final ItemStack chestplate = player.getInventory().getChestplate();
        if (chestplate != null && this.C(chestplate)) {
            player.getInventory().setChestplate((ItemStack)null);
        }
        final ItemStack leggings = player.getInventory().getLeggings();
        if (leggings != null && this.C(leggings)) {
            player.getInventory().setLeggings((ItemStack)null);
        }
        final ItemStack boots = player.getInventory().getBoots();
        if (boots != null && this.C(boots)) {
